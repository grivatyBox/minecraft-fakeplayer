...
public interface IBlockAccess extends LevelHeightAccessor {
    ...
    static Iterable<BlockPosition> boxTraverseBlocks(Vec3D vec3d, Vec3D vec3d1, AxisAlignedBB axisalignedbb) {
        ...
        if (vec3d2.lengthSqr() < (double) MathHelper.square(0.99999F)) {
            ...
            Vec3D vec3d3 = vec3d2.normalize().scale(1.0E-7D);
            Vec3D vec3d4 = axisalignedbb.getMinPosition().add(vec3d3);
            Vec3D vec3d5 = axisalignedbb.getMinPosition().subtract(vec3d2).subtract(vec3d3);

            addCollisionsAlongTravel(set, vec3d5, vec3d4, axisalignedbb);
