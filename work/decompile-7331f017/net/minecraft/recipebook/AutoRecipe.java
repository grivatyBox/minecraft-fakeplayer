...
public class AutoRecipe<R extends IRecipe<?>> {
    ...
    private void placeRecipe(RecipeHolder<R> recipeholder, StackedItemContents stackeditemcontents) {
        ...
        if (stackeditemcontents.canCraft(irecipe, j, list::add)) {
            OptionalInt optionalint = list.stream().mapToInt((holder) -> {
                return ((Item) holder.value()).getDefaultMaxStackSize();
            }).min();
            ...
            if (stackeditemcontents.canCraft(irecipe, j, list::add)) {
                this.clearGrid();
                PlaceRecipeHelper.placeRecipe(this.gridWidth, this.gridHeight, recipeholder.value(), recipeholder.value().placementInfo().slotInfo(), (optional, k, l, i1) -> {
                    if (!optional.isEmpty()) {
                        Slot slot1 = (Slot) this.inputGridSlots.get(k);
                        int j1 = ((PlacementInfo.a) optional.get()).placerOutputPosition();
                        int k1 = j;
                        ...
                        do {
                            ...
                            Holder<Item> holder = (Holder) list.get(j1);
