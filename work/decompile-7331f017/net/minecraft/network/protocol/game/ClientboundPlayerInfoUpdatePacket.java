...
public class ClientboundPlayerInfoUpdatePacket implements Packet<PacketListenerPlayOut> {
    ...
    public static record b(UUID profileId, @Nullable GameProfile profile, boolean listed, int latency, EnumGamemode gameMode, @Nullable IChatBaseComponent displayName, int listOrder, @Nullable RemoteChatSession.a chatSession) {

        b(EntityPlayer entityplayer) {
            this(entityplayer.getUUID(), entityplayer.getGameProfile(), true, entityplayer.connection.latency(), entityplayer.gameMode.getGameModeForPlayer(), entityplayer.getTabListDisplayName(), entityplayer.getTabListOrder(), (RemoteChatSession.a) Optionull.map(entityplayer.getChatSession(), RemoteChatSession::asData));
        }
    }
    ...
    private static class c {
        ...
        ClientboundPlayerInfoUpdatePacket.b build() {
            return new ClientboundPlayerInfoUpdatePacket.b(this.profileId, this.profile, this.listed, this.latency, this.gameMode, this.displayName, this.listOrder, this.chatSession);
        }
    }
}
