...
public record RecipeDisplayEntry(RecipeDisplayId id, RecipeDisplay display, OptionalInt group, RecipeBookCategory category, Optional<List<RecipeItemStack>> craftingRequirements) {
    ...
    public boolean canCraft(StackedItemContents stackeditemcontents) {
        if (this.craftingRequirements.isEmpty()) {
            ...
            List<AutoRecipeStackManager.a<Holder<Item>>> list = ((List) this.craftingRequirements.get()).stream().map(PlacementInfo::ingredientToContents).toList();

            return stackeditemcontents.canCraft(list, (AutoRecipeStackManager.b) null);
        }
    }
}
