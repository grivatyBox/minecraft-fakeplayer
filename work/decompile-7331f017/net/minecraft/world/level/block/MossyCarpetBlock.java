...
public class MossyCarpetBlock extends Block implements IBlockFragilePlantElement {
    ...
    private static boolean canSupportAtFace(IBlockAccess iblockaccess, BlockPosition blockposition, EnumDirection enumdirection) {
        if (enumdirection == EnumDirection.UP) {
            ...
            return MultifaceBlock.canAttachTo(iblockaccess, enumdirection, blockposition1, iblockaccess.getBlockState(blockposition1));
        }
    }
    ...
    public boolean isValidBonemealTarget(IWorldReader iworldreader, BlockPosition blockposition, IBlockData iblockdata) {
        return (Boolean) iblockdata.getValue(MossyCarpetBlock.BASE);
    }

    @Override
    public boolean isBonemealSuccess(World world, RandomSource randomsource, BlockPosition blockposition, IBlockData iblockdata) {
        return !createTopperWithSideChance(world, blockposition, () -> {
            return true;
        }).isAir();
    }
