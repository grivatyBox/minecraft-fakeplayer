...
public class TileEntityCampfire extends TileEntity implements Clearable {
    ...
    protected void loadAdditional(ValueInput valueinput) {
        ...
        valueinput.getIntArray("CookingTimes").ifPresentOrElse((aint) -> {
            System.arraycopy(aint, 0, this.cookingProgress, 0, Math.min(this.cookingTime.length, aint.length));
        }, () -> {
            Arrays.fill(this.cookingProgress, 0);
        });
        valueinput.getIntArray("CookingTotalTimes").ifPresentOrElse((aint) -> {
    ...
    public NBTTagCompound getUpdateTag(HolderLookup.a holderlookup_a) {
        try (ProblemReporter.j problemreporter_j = new ProblemReporter.j(this.problemPath(), TileEntityCampfire.LOGGER)) {
