...
public interface VoxelShapeCollision {
    ...
    static VoxelShapeCollision placementContext(@Nullable EntityHuman entityhuman) {
        return new VoxelShapeCollisionEntity(entityhuman != null ? entityhuman.isDescending() : false, true, entityhuman != null ? entityhuman.getY() : -Double.MAX_VALUE, entityhuman instanceof EntityLiving ? ((EntityLiving) entityhuman).getMainHandItem() : ItemStack.EMPTY, entityhuman instanceof EntityLiving ? (fluid) -> {
            return ((EntityLiving) entityhuman).canStandOnFluid(fluid);
        } : (fluid) -> {
            return false;
        }, entityhuman);
    }
    ...
    static VoxelShapeCollision withPosition(@Nullable Entity entity, double d0) {
        ...
        voxelshapecollisionentity.<init>(flag, true, d1, itemstack, predicate, entity);
