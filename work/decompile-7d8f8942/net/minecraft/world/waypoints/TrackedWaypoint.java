...
public abstract class TrackedWaypoint implements Waypoint {
    ...
    public abstract double yawAngleToCamera(World world, TrackedWaypoint.b trackedwaypoint_b);

    public abstract TrackedWaypoint.e pitchDirectionToCamera(World world, TrackedWaypoint.f trackedwaypoint_f);
    ...
    private static class d extends TrackedWaypoint {
        ...
        @Override
        public double yawAngleToCamera(World world, TrackedWaypoint.b trackedwaypoint_b) {
            return Double.NaN;
        }

        @Override
        public TrackedWaypoint.e pitchDirectionToCamera(World world, TrackedWaypoint.f trackedwaypoint_f) {
    ...
    private static class h extends TrackedWaypoint {
        ...
        private Vec3D position(World world) {
            ...
            return (Vec3D) optional.map(world::getEntity).map((entity) -> {
                return entity.blockPosition().distManhattan(this.vector) > 3 ? null : entity.getEyePosition();
            }).orElseGet(() -> {
        ...
        @Override
        public double yawAngleToCamera(World world, TrackedWaypoint.b trackedwaypoint_b) {
            Vec3D vec3d = trackedwaypoint_b.position().subtract(this.position(world)).rotateClockwise90();
        ...
        @Override
        public TrackedWaypoint.e pitchDirectionToCamera(World world, TrackedWaypoint.f trackedwaypoint_f) {
            Vec3D vec3d = trackedwaypoint_f.projectPointToScreen(this.position(world));
