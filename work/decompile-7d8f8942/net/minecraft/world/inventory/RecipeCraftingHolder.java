...
public interface RecipeCraftingHolder {
    ...
    default boolean setRecipeUsed(EntityPlayer entityplayer, RecipeHolder<?> recipeholder) {
        if (!recipeholder.value().isSpecial() && entityplayer.level().getGameRules().getBoolean(GameRules.RULE_LIMITED_CRAFTING) && !entityplayer.getRecipeBook().contains(recipeholder.id())) {
            return false;
        } else {
