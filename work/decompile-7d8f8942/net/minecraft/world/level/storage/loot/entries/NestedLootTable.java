...
public class NestedLootTable extends LootSelectorEntry {
    ...
    public static final ProblemReporter.f INLINE_LOOT_TABLE_PATH_ELEMENT = new ProblemReporter.f() {
        @Override
        public String get() {
            return "->{inline}";
        }
    ...
    public void validate(LootCollector lootcollector) {
        ...
        if (optional.isPresent()) {
            ...
            if (!lootcollector.allowsReferences()) {
                lootcollector.reportProblem(new LootCollector.d(resourcekey));
                return;
            }

            if (lootcollector.hasVisitedElement(resourcekey)) {
                lootcollector.reportProblem(new LootCollector.c(resourcekey));
                return;
            }
        }
        ...
        this.contents.ifLeft((resourcekey1) -> {
            lootcollector.resolver().get(resourcekey1).ifPresentOrElse((holder_c) -> {
                ((LootTable) holder_c.value()).validate(lootcollector.enterElement(new ProblemReporter.b(resourcekey1), resourcekey1));
            }, () -> {
                lootcollector.reportProblem(new LootCollector.a(resourcekey1));
            });
        }).ifRight((loottable) -> {
            loottable.validate(lootcollector.forChild(NestedLootTable.INLINE_LOOT_TABLE_PATH_ELEMENT));
        });
    }
