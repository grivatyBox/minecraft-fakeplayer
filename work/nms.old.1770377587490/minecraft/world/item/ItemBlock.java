...
public class ItemBlock extends Item {
    ...
    public static boolean updateCustomBlockEntityTag(World world, @Nullable EntityHuman entityhuman, BlockPosition blockposition, ItemStack itemstack) {
        if (world.isClientSide()) {
            ...
            TypedEntityData<TileEntityTypes<?>> typedentitydata = (TypedEntityData) itemstack.get(DataComponents.BLOCK_ENTITY_DATA);

            if (typedentitydata != null) {
                TileEntity tileentity = world.getBlockEntity(blockposition);

                if (tileentity != null) {
                    TileEntityTypes<?> tileentitytypes = tileentity.getType();

                    if (tileentitytypes != typedentitydata.type()) {
                        return false;
                    }

                    if (!tileentitytypes.onlyOpCanSetNbt() || entityhuman != null && entityhuman.canUseGameMasterBlocks()) {
    ...
    public boolean shouldPrintOpWarning(ItemStack itemstack, @Nullable EntityHuman entityhuman) {
        if (entityhuman != null && entityhuman.getPermissionLevel() >= 2) {
            ...
            if (typedentitydata != null) {
                return ((TileEntityTypes) typedentitydata.type()).onlyOpCanSetNbt();
            }
        }
