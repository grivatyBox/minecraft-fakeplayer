...
public class EntityItemFrame extends EntityHanging {
    ...
    protected AxisAlignedBB calculateBoundingBox(BlockPosition blockposition, EnumDirection enumdirection) {
        return this.createBoundingBox(blockposition, enumdirection, this.hasFramedMap());
    }
    ...
    public static AxisAlignedBB createBoundingBox(BlockPosition blockposition, EnumDirection enumdirection, boolean flag) { // CraftBukkit - static
        float f = 0.46875F;
        Vec3D vec3d = Vec3D.atCenterOf(blockposition).relative(enumdirection, -0.46875D);
        float f1 = flag ? 1.0F : 0.75F;
        float f2 = flag ? 1.0F : 0.75F;
        EnumDirection.EnumAxis enumdirection_enumaxis = enumdirection.getAxis();
        double d0 = enumdirection_enumaxis == EnumDirection.EnumAxis.X ? 0.0625D : (double) f1;
        double d1 = enumdirection_enumaxis == EnumDirection.EnumAxis.Y ? 0.0625D : (double) f2;
        double d2 = enumdirection_enumaxis == EnumDirection.EnumAxis.Z ? 0.0625D : (double) f1;
    ...
    public boolean survives() {
        if (this.fixed) {
            ...
            return iblockdata.isSolid() || this.getDirection().getAxis().isHorizontal() && BlockDiodeAbstract.isDiode(iblockdata) ? this.canCoexist(true) : false;
        }
    }
