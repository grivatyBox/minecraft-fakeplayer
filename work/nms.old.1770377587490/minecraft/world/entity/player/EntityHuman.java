...
public abstract class EntityHuman extends Avatar implements ContainerUser {
    ...
    private static final DataWatcherObject<OptionalInt> DATA_SHOULDER_PARROT_LEFT = DataWatcher.<OptionalInt>defineId(EntityHuman.class, DataWatcherRegistry.OPTIONAL_UNSIGNED_INT);
    private static final DataWatcherObject<OptionalInt> DATA_SHOULDER_PARROT_RIGHT = DataWatcher.<OptionalInt>defineId(EntityHuman.class, DataWatcherRegistry.OPTIONAL_UNSIGNED_INT);
    ...
    protected void defineSynchedData(DataWatcher.a datawatcher_a) {
        ...
        datawatcher_a.define(EntityHuman.DATA_SHOULDER_PARROT_LEFT, OptionalInt.empty());
        datawatcher_a.define(EntityHuman.DATA_SHOULDER_PARROT_RIGHT, OptionalInt.empty());
    }
        ...
        if (entity.isAttackable()) {
            if (!entity.skipAttackInteraction(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    boolean flag2 = flag && this.fallDistance > 0.0D && !this.onGround() && !this.onClimbable() && !this.isInWater() && !this.isMobilityRestricted() && !this.isPassenger() && entity instanceof EntityLiving && !this.isSprinting();
                    ...
                    if (flag4) {
                        ...
                        if (f5 > 0.0F) {
                            if (entity instanceof EntityLiving) {
                                ...
                                entityliving1.knockback((double) (f5 * 0.5F), (double) MathHelper.sin(this.getYRot() * ((float) Math.PI / 180F)), (double) (-MathHelper.cos(this.getYRot() * ((float) Math.PI / 180F))), this, org.bukkit.event.entity.EntityKnockbackEvent.KnockbackCause.ENTITY_ATTACK); // CraftBukkit
                            } else {
                                    ...
                                    if (this.distanceToSqr((Entity) entityliving2) < 9.0D) {
                                        ...
                                        if (world instanceof WorldServer) {
                                            ...
                                            if (entityliving2.hurtServer(worldserver, damagesource, f7)) {
                                                entityliving2.knockback((double) 0.4F, (double) MathHelper.sin(this.getYRot() * ((float) Math.PI / 180F)), (double) (-MathHelper.cos(this.getYRot() * ((float) Math.PI / 180F))), this, org.bukkit.event.entity.EntityKnockbackEvent.KnockbackCause.SWEEP_ATTACK); // CraftBukkit
    ...
    public IChatBaseComponent getName() {
        return IChatBaseComponent.literal(this.gameProfile.name());
    }
    ...
    protected static OptionalInt convertParrotVariant(Optional<EntityParrot.Variant> optional) {
        return (OptionalInt) optional.map((entityparrot_variant) -> {
            return OptionalInt.of(entityparrot_variant.getId());
        }).orElse(OptionalInt.empty());
    }

    private static Optional<EntityParrot.Variant> convertParrotVariant(OptionalInt optionalint) {
        return optionalint.isPresent() ? Optional.of(EntityParrot.Variant.byId(optionalint.getAsInt())) : Optional.empty();
    }

    public void setShoulderParrotLeft(Optional<EntityParrot.Variant> optional) {
        this.entityData.set(EntityHuman.DATA_SHOULDER_PARROT_LEFT, convertParrotVariant(optional));
    }

    public Optional<EntityParrot.Variant> getShoulderParrotLeft() {
        return convertParrotVariant((OptionalInt) this.entityData.get(EntityHuman.DATA_SHOULDER_PARROT_LEFT));
    }

    public void setShoulderParrotRight(Optional<EntityParrot.Variant> optional) {
        this.entityData.set(EntityHuman.DATA_SHOULDER_PARROT_RIGHT, convertParrotVariant(optional));
    }

    public Optional<EntityParrot.Variant> getShoulderParrotRight() {
        return convertParrotVariant((OptionalInt) this.entityData.get(EntityHuman.DATA_SHOULDER_PARROT_RIGHT));
    }
