...
public class EntityExperienceOrb extends Entity {
    ...
    public EntityExperienceOrb(World world, double d0, double d1, double d2, int i) {
        this(world, new Vec3D(d0, d1, d2), Vec3D.ZERO, i);
    }

    public EntityExperienceOrb(World world, Vec3D vec3d, Vec3D vec3d1, int i) {
        this(EntityTypes.EXPERIENCE_ORB, world);
        this.setPos(vec3d);
        if (!world.isClientSide()) {
            this.setYRot(this.random.nextFloat() * 360.0F);
            Vec3D vec3d2 = new Vec3D((this.random.nextDouble() * 0.2D - 0.1D) * 2.0D, this.random.nextDouble() * 0.2D * 2.0D, (this.random.nextDouble() * 0.2D - 0.1D) * 2.0D);

            if (vec3d1.lengthSqr() > 0.0D && vec3d1.dot(vec3d2) < 0.0D) {
                vec3d2 = vec3d2.scale(-1.0D);
            }

            double d0 = this.getBoundingBox().getSize();

            this.setPos(vec3d.add(vec3d1.normalize().scale(d0 * 0.5D)));
    ...
    protected void unstuckIfPossible(double d0) {
        Vec3D vec3d = this.position().add(0.0D, (double) this.getBbHeight() / 2.0D, 0.0D);
        VoxelShape voxelshape = VoxelShapes.create(AxisAlignedBB.ofSize(vec3d, d0, d0, d0));

        this.level().findFreePosition(this, voxelshape, vec3d, (double) this.getBbWidth(), (double) this.getBbHeight(), (double) this.getBbWidth()).ifPresent((vec3d1) -> {
            this.setPos(vec3d1.add(0.0D, (double) (-this.getBbHeight()) / 2.0D, 0.0D));
        });
    }
    ...
    public static void award(WorldServer worldserver, Vec3D vec3d, int i) {
        awardWithDirection(worldserver, vec3d, Vec3D.ZERO, i);
    }

    public static void awardWithDirection(WorldServer worldserver, Vec3D vec3d, Vec3D vec3d1, int i) {
        while (i > 0) {
            int j = getExperienceValue(i);

            i -= j;
            if (!tryMergeToExisting(worldserver, vec3d, j)) {
                worldserver.addFreshEntity(new EntityExperienceOrb(worldserver, vec3d, vec3d1, j));
            }
        }
