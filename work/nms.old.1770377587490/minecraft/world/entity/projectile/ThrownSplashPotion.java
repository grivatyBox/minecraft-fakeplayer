...
public class ThrownSplashPotion extends EntityPotion {
    ...
    @Override
    public void onHitAsPotion(WorldServer worldserver, ItemStack itemstack, MovingObjectPosition movingobjectposition) {
        ...
        AxisAlignedBB axisalignedbb = this.getBoundingBox().move(movingobjectposition.getLocation().subtract(this.position()));
        ...
        if (!list.isEmpty()) {
            ...
            for (EntityLiving entityliving : list) {
                if (entityliving.isAffectedByPotions()) {
                    double d0 = axisalignedbb.distanceToSqr(entityliving.getBoundingBox().inflate((double) f1));
                ...
                if (!event.isCancelled() && list != null && !list.isEmpty()) { // do not process effects if there are no effects to process
                    ...
                    for (LivingEntity victim : event.getAffectedEntities()) {
                        ...
                        for (MobEffect mobeffect : iterable) {
                            ...
                            if (!this.level().getWorld().getPVP() && this.getOwner() instanceof EntityPlayer && entityliving instanceof EntityPlayer && entityliving != this.getOwner()) {
