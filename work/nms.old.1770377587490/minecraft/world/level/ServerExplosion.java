...
public class ServerExplosion implements Explosion {
    ...
    private void hurtEntities() {
        ...
        for (Entity entity :list) {
            ...
            if (!entity.ignoreExplosion(this)) {
                ...
                if (d0 <= 1.0D) {
                    Vec3D vec3d = entity instanceof EntityTNTPrimed ? entity.position() : entity.getEyePosition();
                    Vec3D vec3d1 = vec3d.subtract(this.center).normalize();
                    ...
                    if (entity instanceof EntityLiving) {
                        ...
                        d1 = entityliving.getAttributeValue(GenericAttributes.EXPLOSION_KNOCKBACK_RESISTANCE);
                    } else {
                        d1 = 0.0D;
                    }
                    ...
                    Vec3D vec3d2 = vec3d1.scale(d3);
                    // CraftBukkit start - Call EntityKnockbackEvent
                    if (entity instanceof EntityLiving) {
                        Vec3D result = entity.getDeltaMovement().add(vec3d2);
                        org.bukkit.event.entity.EntityKnockbackEvent event = CraftEventFactory.callEntityKnockbackEvent((org.bukkit.craftbukkit.entity.CraftLivingEntity) entity.getBukkitEntity(), source, org.bukkit.event.entity.EntityKnockbackEvent.KnockbackCause.EXPLOSION, d3, vec3d2, result.x, result.y, result.z);
                        ...
                        vec3d2 = (event.isCancelled()) ? Vec3D.ZERO : new Vec3D(event.getFinalKnockback().getX(), event.getFinalKnockback().getY(), event.getFinalKnockback().getZ()).subtract(entity.getDeltaMovement());
                    }
                    // CraftBukkit end
                    ...
                    if (entity.getType().is(TagsEntity.REDIRECTABLE_PROJECTILE) && entity instanceof IProjectile) {
                        ...
                        iprojectile.setOwner(this.damageSource.getEntity());
    ...
    public boolean shouldAffectBlocklikeEntities() {
        ...
        boolean flag1 = this.source == null || this.source.getType() != EntityTypes.BREEZE_WIND_CHARGE && this.source.getType() != EntityTypes.WIND_CHARGE;
