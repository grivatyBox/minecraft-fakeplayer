...
public class TileEntityBeehive extends TileEntity {
    ...
    public void registerDebugValues(WorldServer worldserver, DebugValueSource.a debugvaluesource_a) {
        debugvaluesource_a.register(DebugSubscriptions.BEE_HIVES, () -> {
    ...
    public static record c(TypedEntityData<EntityTypes<?>> entityData, int ticksInHive, int minTicksInHive) {

        public static final Codec<TileEntityBeehive.c> CODEC = RecordCodecBuilder.create((instance) -> {
            return instance.group(TypedEntityData.codec(EntityTypes.CODEC).fieldOf("entity_data").forGetter(TileEntityBeehive.c::entityData), Codec.INT.fieldOf("ticks_in_hive").forGetter(TileEntityBeehive.c::ticksInHive), Codec.INT.fieldOf("min_ticks_in_hive").forGetter(TileEntityBeehive.c::minTicksInHive)).apply(instance, TileEntityBeehive.c::new);
        });
        public static final Codec<List<TileEntityBeehive.c>> LIST_CODEC = TileEntityBeehive.c.CODEC.listOf();
        public static final StreamCodec<RegistryFriendlyByteBuf, TileEntityBeehive.c> STREAM_CODEC = StreamCodec.composite(TypedEntityData.streamCodec(EntityTypes.STREAM_CODEC), TileEntityBeehive.c::entityData, ByteBufCodecs.VAR_INT, TileEntityBeehive.c::ticksInHive, ByteBufCodecs.VAR_INT, TileEntityBeehive.c::minTicksInHive, TileEntityBeehive.c::new);
        ...
        public static TileEntityBeehive.c of(Entity entity) {
            ...
            try (ProblemReporter.j problemreporter_j = new ProblemReporter.j(entity.problemPath(), TileEntityBeehive.LOGGER)) {
                ...
                tileentitybeehive_c = new TileEntityBeehive.c(TypedEntityData.of(entity.getType(), nbttagcompound), 0, flag ? 2400 : 600);
            }
        ...
        public static TileEntityBeehive.c create(int i) {
            return new TileEntityBeehive.c(TypedEntityData.of(EntityTypes.BEE, new NBTTagCompound()), i, 600);
        }

        @Nullable
        public Entity createEntity(World world, BlockPosition blockposition) {
            NBTTagCompound nbttagcompound = this.entityData.copyTagWithoutId();
            List<String> list = TileEntityBeehive.IGNORED_BEE_TAGS; // CraftBukkit - decompile error

            Objects.requireNonNull(nbttagcompound);
            list.forEach(nbttagcompound::remove);
            Entity entity = EntityTypes.loadEntityRecursive(this.entityData.type(), nbttagcompound, world, EntitySpawnReason.LOAD, (entity1) -> {
                return entity1;
            });
