...
public class BlockEnderPortal extends BlockTileEntity implements Portal {
    ...
    public TeleportTransition getPortalDestination(WorldServer worldserver, Entity entity, BlockPosition blockposition) {
        ...
        // CraftBukkit start - SPIGOT-6152: send back to main overworld in custom ends
        ResourceKey<WorldDimension> resourcekey = worldserver.getTypeKey();
        boolean flag = resourcekey == WorldDimension.END;
        // CraftBukkit end
        ResourceKey<World> resourcekey1 = flag ? worlddata_a.dimension() : World.END;
        BlockPosition blockposition1 = flag ? worlddata_a.pos() : WorldServer.END_SPAWN_POINT;
        WorldServer worldserver1 = worldserver.getServer().getLevel(resourcekey1);
        ...
        if (worldserver1 == null) {
            ...
            // CraftBukkit start
            CraftPortalEvent event = entity.callPortalEvent(entity, CraftLocation.toBukkit(vec3d, worldserver1.getWorld(), f, f1), PlayerTeleportEvent.TeleportCause.END_PORTAL, 0, 0);
            if (event == null) {
                return null;
            }
