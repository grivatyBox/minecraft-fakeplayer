...
public class BlockSweetBerryBush extends VegetationBlock implements IBlockFragilePlantElement {
    ...
    @Override
    protected void entityInside(IBlockData iblockdata, World world, BlockPosition blockposition, Entity entity, InsideBlockEffectApplier insideblockeffectapplier, boolean flag) {
    ...
    protected EnumInteractionResult useWithoutItem(IBlockData iblockdata, World world, BlockPosition blockposition, EntityHuman entityhuman, MovingObjectPositionBlock movingobjectpositionblock) {
        if ((Integer) iblockdata.getValue(BlockSweetBerryBush.AGE) > 1) {
            if (world instanceof WorldServer) {
                WorldServer worldserver = (WorldServer) world;

                // CraftBukkit start - useWithoutItem is always MAIN_HAND
                List<ItemStack> dropped = new LinkedList<>();
                Block.dropFromBlockInteractLootTable(worldserver, LootTables.HARVEST_SWEET_BERRY_BUSH, iblockdata, world.getBlockEntity(blockposition), (ItemStack) null, entityhuman, (worldserver1, itemstack) -> {
                    dropped.add(itemstack);
                });
                PlayerHarvestBlockEvent event = CraftEventFactory.callPlayerHarvestBlockEvent(world, blockposition, entityhuman, EnumHand.MAIN_HAND, dropped);
                ...
                // CraftBukkit end
                worldserver.playSound((Entity) null, blockposition, SoundEffects.SWEET_BERRY_BUSH_PICK_BERRIES, SoundCategory.BLOCKS, 1.0F, 0.8F + worldserver.random.nextFloat() * 0.4F);
                ...
                worldserver.gameEvent(GameEvent.BLOCK_CHANGE, blockposition, GameEvent.a.of(entityhuman, iblockdata1));
            }
