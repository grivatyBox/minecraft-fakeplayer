...
public record SerializableChunkData(PalettedContainerFactory containerFactory, ChunkCoordIntPair chunkPos, int minSectionY, long lastUpdateTime, long inhabitedTime, ChunkStatus chunkStatus, @Nullable BlendingData.d blendingData, @Nullable BelowZeroRetrogen belowZeroRetrogen, ChunkConverter upgradeData, @Nullable long[] carvingMask, Map<HeightMap.Type, long[]> heightmaps, IChunkAccess.b packedTicks, ShortList[] postProcessingSections, boolean lightCorrect, List<SerializableChunkData.b> sectionData, List<NBTTagCompound> entities, List<NBTTagCompound> blockEntities, NBTTagCompound structureData, @Nullable NBTBase persistentDataContainer) {
    ...
    @Nullable
    public static SerializableChunkData parse(LevelHeightAccessor levelheightaccessor, PalettedContainerFactory palettedcontainerfactory, NBTTagCompound nbttagcompound) {
        if (nbttagcompound.getString("Status").isEmpty()) {
            ...
            Codec<DataPaletteBlock<Holder<BiomeBase>>> codec = palettedcontainerfactory.biomeContainerCodecRW(); // CraftBukkit - read/write
            Codec<DataPaletteBlock<IBlockData>> codec1 = palettedcontainerfactory.blockStatesContainerCodec();
            ...
            for (int i1 = 0; i1 < nbttaglist2.size(); ++i1) {
                ...
                if (!optional.isEmpty()) {
                    ...
                    if (j1 >= levelheightaccessor.getMinSectionY() && j1 <= levelheightaccessor.getMaxSectionY()) {
                        Optional optional1 = nbttagcompound2.getCompound("block_states").map((nbttagcompound3) -> {
                            return (DataPaletteBlock) codec1.parse(DynamicOpsNBT.INSTANCE, nbttagcompound3).promotePartial((s) -> {
                                logErrors(chunkcoordintpair, j1, s);
                            }).getOrThrow(SerializableChunkData.a::new);
                        });

                        Objects.requireNonNull(palettedcontainerfactory);
                        DataPaletteBlock<IBlockData> datapaletteblock = (DataPaletteBlock) optional1.orElseGet(palettedcontainerfactory::createForBlockStates);

                        optional1 = nbttagcompound2.getCompound("biomes").map((nbttagcompound3) -> {
                            return (PalettedContainerRO) codec.parse(DynamicOpsNBT.INSTANCE, nbttagcompound3).promotePartial((s) -> {
                                logErrors(chunkcoordintpair, j1, s);
                            }).getOrThrow(SerializableChunkData.a::new);
                        });
                        Objects.requireNonNull(palettedcontainerfactory);
                        DataPaletteBlock<Holder<BiomeBase>> palettedcontainerro = (DataPaletteBlock) optional1.orElseGet(palettedcontainerfactory::createForBiomes); // CraftBukkit - read/write
            ...
            return new SerializableChunkData(palettedcontainerfactory, chunkcoordintpair, levelheightaccessor.getMinSectionY(), i, j, chunkstatus, blendingdata_d, belowzeroretrogen, chunkconverter, along, map, ichunkaccess_b, ashortlist, flag, list4, list2, list3, nbttagcompound1, nbttagcompound.get("ChunkBukkitValues"));
        }
    }
    ...
    public NBTTagCompound write() {
        ...
        Codec<DataPaletteBlock<IBlockData>> codec = this.containerFactory.blockStatesContainerCodec();
        Codec<PalettedContainerRO<Holder<BiomeBase>>> codec1 = this.containerFactory.biomeContainerCodec();
