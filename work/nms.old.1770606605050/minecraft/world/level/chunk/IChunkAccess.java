...
public abstract class IChunkAccess implements BiomeManager.Provider, LightChunk, StructureAccess {
    ...
    public IChunkAccess(ChunkCoordIntPair chunkcoordintpair, ChunkConverter chunkconverter, LevelHeightAccessor levelheightaccessor, PalettedContainerFactory palettedcontainerfactory, long i, @Nullable ChunkSection[] achunksection, @Nullable BlendingData blendingdata) {
    ...
    private static void replaceMissingSections(PalettedContainerFactory palettedcontainerfactory, ChunkSection[] achunksection) {
        for (int i = 0; i < achunksection.length; ++i) {
            if (achunksection[i] == null) {
                achunksection[i] = new ChunkSection(palettedcontainerfactory);
            }
        }
    ...
    public int getHeight(HeightMap.Type heightmap_type, int i, int j) {
        ...
        if (heightmap == null) {
            if (SharedConstants.IS_RUNNING_IN_IDE && this instanceof Chunk) {
                IChunkAccess.LOGGER.error("Unprimed heightmap: {} {} {}", new Object[]{heightmap_type, i, j});
            }
