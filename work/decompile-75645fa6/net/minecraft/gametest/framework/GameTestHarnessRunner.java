...
public class GameTestHarnessRunner {
    ...
    protected GameTestHarnessRunner(GameTestHarnessRunner.b gametestharnessrunner_b, Collection<GameTestHarnessBatch> collection, WorldServer worldserver, GameTestHarnessTicker gametestharnessticker, GameTestHarnessRunner.c gametestharnessrunner_c, GameTestHarnessRunner.c gametestharnessrunner_c1, boolean flag, boolean flag1) {
        ...
        this.clearBetweenBatches = flag1;
    ...
    void runBatch(final int i) {
        if (i >= this.batches.size()) {
            ...
            if (i > 0 && this.clearBetweenBatches) {
                GameTestHarnessBatch gametestharnessbatch = (GameTestHarnessBatch) this.batches.get(i - 1);

                gametestharnessbatch.gameTestInfos().forEach((gametestharnessinfo) -> {
                    TestInstanceBlockEntity testinstanceblockentity = gametestharnessinfo.getTestInstanceBlockEntity();

                    GameTestHarnessStructures.clearSpaceForStructure(testinstanceblockentity.getStructureBoundingBox(), this.level);
                    this.level.destroyBlock(testinstanceblockentity.getBlockPos(), false);
                });
            }

            final GameTestHarnessBatch gametestharnessbatch1 = (GameTestHarnessBatch) this.batches.get(i);

            this.existingStructureSpawner.onBatchStart(this.level);
            this.newStructureSpawner.onBatchStart(this.level);
            Collection<GameTestHarnessInfo> collection = this.createStructuresForBatch(gametestharnessbatch1.gameTestInfos());

            GameTestHarnessRunner.LOGGER.info("Running test environment '{}' batch {} ({} tests)...", new Object[]{gametestharnessbatch1.environment().getRegisteredName(), gametestharnessbatch1.index(), collection.size()});
            this.endCurrentEnvironment();
            this.currentEnvironment = gametestharnessbatch1.environment();
            ...
            this.batchListeners.forEach((gametestbatchlistener) -> {
                gametestbatchlistener.testBatchStarting(gametestharnessbatch1);
            });
            ...
            gametestharnesscollector.addListener(new GameTestHarnessListener() {
                private void testCompleted(GameTestHarnessInfo gametestharnessinfo) {
                    gametestharnessinfo.getTestInstanceBlockEntity().removeBarriers();
                    if (gametestharnesscollector.isDone()) {
                        GameTestHarnessRunner.this.batchListeners.forEach((gametestbatchlistener) -> {
                            gametestbatchlistener.testBatchFinished(gametestharnessbatch1);
                        });
                ...
                public void testPassed(GameTestHarnessInfo gametestharnessinfo, GameTestHarnessRunner gametestharnessrunner) {
                    this.testCompleted(gametestharnessinfo);
                }
    ...
    public interface c {
        ...
        GameTestHarnessRunner.c IN_PLACE = (gametestharnessinfo) -> {
            return Optional.ofNullable(gametestharnessinfo.prepareTestStructure()).map((gametestharnessinfo1) -> {
                return gametestharnessinfo1.startExecution(1);
            });
        };
    ...
    public static class a {
        ...
        public GameTestHarnessRunner.a haltOnError() {
            this.haltOnError = true;
            return this;
        }

        public GameTestHarnessRunner.a clearBetweenBatches() {
        ...
        public GameTestHarnessRunner build() {
            return new GameTestHarnessRunner(this.batcher, this.batches, this.level, this.testTicker, this.existingStructureSpawner, this.newStructureSpawner, this.haltOnError, this.clearBetweenBatches);
        }
    }
