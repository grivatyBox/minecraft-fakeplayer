...
public class BlockBasedTestInstance extends GameTestInstance {
    ...
    public void run(GameTestHarnessHelper gametestharnesshelper) {
        ...
        gametestharnesshelper.onEachTick(() -> {
            ...
            if (list.isEmpty()) {
                gametestharnesshelper.fail((IChatBaseComponent) IChatBaseComponent.translatable("test_block.error.missing", TestBlockMode.ACCEPT.getDisplayName()));
            }
            ...
            if (flag) {
                ...
                this.forAllTriggeredTestBlocks(gametestharnesshelper, TestBlockMode.FAIL, (testblockentity1) -> {
                    gametestharnesshelper.fail((IChatBaseComponent) IChatBaseComponent.literal(testblockentity1.getMessage()));
                });
    ...
    private BlockPosition findStartBlock(GameTestHarnessHelper gametestharnesshelper) {
        ...
        if (list.isEmpty()) {
            gametestharnesshelper.fail((IChatBaseComponent) IChatBaseComponent.translatable("test_block.error.missing", TestBlockMode.START.getDisplayName()));
        }

        if (list.size() != 1) {
            gametestharnesshelper.fail((IChatBaseComponent) IChatBaseComponent.translatable("test_block.error.too_many", TestBlockMode.START.getDisplayName()));
        }
