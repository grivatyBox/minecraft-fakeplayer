...
public class GameTestHarnessTestCommand {
    ...
    private static int clear(TestFinder testfinder) throws CommandSyntaxException {
        ...
        List<TestInstanceBlockEntity> list = testfinder.findTestPos().flatMap((blockposition) -> {
        ...
        for (TestInstanceBlockEntity testinstanceblockentity : list) {
            GameTestHarnessStructures.clearSpaceForStructure(testinstanceblockentity.getStructureBoundingBox(), worldserver);
            testinstanceblockentity.removeBarriers();
            worldserver.destroyBlock(testinstanceblockentity.getBlockPos(), false);
        }
    ...
    private static int verify(TestFinder testfinder) {
        ...
        GameTestHarnessRunner gametestharnessrunner = GameTestHarnessRunner.a.fromBatches(collection1, worldserver).batcher(GameTestBatchFactory.fromGameTestInfo(100)).newStructureSpawner(structuregridspawner).existingStructureSpawner(structuregridspawner).haltOnError().clearBetweenBatches().build();
    ...
    public static void register(CommandDispatcher<CommandListenerWrapper> commanddispatcher, CommandBuildContext commandbuildcontext) {
        ...
        LiteralArgumentBuilder literalargumentbuilder = (LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) net.minecraft.commands.CommandDispatcher.literal("test").requires(net.minecraft.commands.CommandDispatcher.hasPermission(2))).then(net.minecraft.commands.CommandDispatcher.literal("run").then(runWithRetryOptionsAndBuildInfo(net.minecraft.commands.CommandDispatcher.argument("tests", ResourceSelectorArgument.resourceSelector(commandbuildcontext, Registries.TEST_INSTANCE)), (commandcontext) -> {
    ...
    private static int showPos(CommandListenerWrapper commandlistenerwrapper, String s) throws CommandSyntaxException {
        ...
        MovingObjectPositionBlock movingobjectpositionblock = (MovingObjectPositionBlock) entityplayer.pick(10.0D, 1.0F, false);
        ...
        if (optional.isEmpty()) {
            optional = GameTestHarnessStructures.findTestContainingPos(blockposition, 250, worldserver);
        }
        ...
        if (optional.isEmpty()) {
            ...
            if (tileentity instanceof TestInstanceBlockEntity) {
                ...
                entityplayer.connection.send(new ClientboundGameTestHighlightPosPacket(blockposition, blockposition2));
                return 1;
            } else {
                throw GameTestHarnessTestCommand.TEST_INSTANCE_COULD_NOT_BE_FOUND.create();
            }
