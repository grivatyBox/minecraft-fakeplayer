...
public class SharedConstants {
    ...
    private static String prefixDebugFlagName(String s) {
        return "MC_DEBUG_" + s;
    }

    private static boolean booleanProperty(String s) {
        String s1 = System.getProperty(s);

        return s1 != null && (s1.isEmpty() || Boolean.parseBoolean(s1));
    }

    private static boolean debugFlag(String s) {
        if (!SharedConstants.DEBUG_ENABLED) {
            return false;
        } else {
            String s1 = prefixDebugFlagName(s);
    ...
    private static int debugIntValue(String s) {
        if (!SharedConstants.DEBUG_ENABLED) {
            return 0;
        } else {
            String s1 = prefixDebugFlagName(s);
