...
public record AttributeModifiersPredicate(Optional<CollectionPredicate<ItemAttributeModifiers.c, AttributeModifiersPredicate.a>> modifiers) implements SingleComponentItemPredicate<ItemAttributeModifiers> {
    ...
    public static record a(Optional<HolderSet<AttributeBase>> attribute, Optional<MinecraftKey> id, CriterionConditionValue.DoubleRange amount, Optional<AttributeModifier.Operation> operation, Optional<EquipmentSlotGroup> slot) implements Predicate<ItemAttributeModifiers.c> {
        ...
        public boolean test(ItemAttributeModifiers.c itemattributemodifiers_c) {
            return this.attribute.isPresent() && !((HolderSet) this.attribute.get()).contains(itemattributemodifiers_c.attribute()) ? false : (this.id.isPresent() && !((MinecraftKey) this.id.get()).equals(itemattributemodifiers_c.modifier().id()) ? false : (!this.amount.matches(itemattributemodifiers_c.modifier().amount()) ? false : (this.operation.isPresent() && this.operation.get() != itemattributemodifiers_c.modifier().operation() ? false : !this.slot.isPresent() || this.slot.get() == itemattributemodifiers_c.slot())));
        }
    }
}
