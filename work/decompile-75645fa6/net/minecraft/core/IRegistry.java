...
public interface IRegistry<T> extends Keyable, HolderLookup.b<T>, Registry<T> {
    ...
    static <R, T extends R> Holder.c<T> registerForHolder(IRegistry<R> iregistry, ResourceKey<R> resourcekey, T t0) {
        return ((IRegistryWritable) iregistry).register(resourcekey, t0, RegistrationInfo.BUILT_IN);
    }

    static <R, T extends R> Holder.c<T> registerForHolder(IRegistry<R> iregistry, MinecraftKey minecraftkey, T t0) {
