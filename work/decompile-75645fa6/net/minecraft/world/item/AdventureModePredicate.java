...
public class AdventureModePredicate {
    ...
    private static boolean areSameBlocks(ShapeDetectorBlock shapedetectorblock, @Nullable ShapeDetectorBlock shapedetectorblock1, boolean flag) {
        if (shapedetectorblock1 != null && shapedetectorblock.getState() == shapedetectorblock1.getState()) {
            if (!flag) {
                ...
                try (ProblemReporter.j problemreporter_j = new ProblemReporter.j(AdventureModePredicate.LOGGER)) {
                    IRegistryCustom iregistrycustom = shapedetectorblock.getLevel().registryAccess();
                    NBTTagCompound nbttagcompound = saveBlockEntity(shapedetectorblock.getEntity(), iregistrycustom, problemreporter_j);
                    NBTTagCompound nbttagcompound1 = saveBlockEntity(shapedetectorblock1.getEntity(), iregistrycustom, problemreporter_j);

                    return Objects.equals(nbttagcompound, nbttagcompound1);
                }
            } else {
                return false;
            }
    ...
    private static NBTTagCompound saveBlockEntity(TileEntity tileentity, IRegistryCustom iregistrycustom, ProblemReporter problemreporter) {
        TagValueOutput tagvalueoutput = TagValueOutput.createWithContext(problemreporter.forChild(tileentity.problemPath()), iregistrycustom);
