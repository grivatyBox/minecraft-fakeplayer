...
public class AnimalPanic<E extends EntityCreature> extends Behavior<E> {
    ...
    public AnimalPanic(float f, int i) {
        this(f, (entitycreature) -> {
            ...
            return AirAndWaterRandomPos.getPos(entitycreature, 5, 4, i, entitycreature.getViewVector(0.0F).x, entitycreature.getViewVector(0.0F).z, (double) ((float) Math.PI / 2F));
        });
    }

    public AnimalPanic(float f, Function<EntityCreature, TagKey<DamageType>> function) {
        this(f, function, (entitycreature) -> {
            return LandRandomPos.getPos(entitycreature, 5, 4);
        });
    }

    public AnimalPanic(float f, Function<EntityCreature, TagKey<DamageType>> function, Function<E, Vec3D> function1) {
    ...
    private Vec3D getPanicPos(E e0, WorldServer worldserver) {
        ...
        return (Vec3D) this.positionGetter.apply(e0);
    }
