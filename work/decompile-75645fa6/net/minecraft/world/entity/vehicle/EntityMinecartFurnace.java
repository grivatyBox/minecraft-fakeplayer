...
public class EntityMinecartFurnace extends EntityMinecartAbstract {
    ...
    public EnumInteractionResult interact(EntityHuman entityhuman, EnumHand enumhand) {
        ...
        if (this.addFuel(entityhuman.position(), itemstack)) {
    ...
    public boolean addFuel(Vec3D vec3d, ItemStack itemstack) {
        if (itemstack.is(TagsItem.FURNACE_MINECART_FUEL) && this.fuel + 3600 <= 32000) {
            this.fuel += 3600;
            if (this.fuel > 0) {
                this.push = this.position().subtract(vec3d).horizontal();
            }
    ...
    protected void readAdditionalSaveData(ValueInput valueinput) {
        ...
        double d0 = valueinput.getDoubleOr("PushX", EntityMinecartFurnace.DEFAULT_PUSH.x);
        double d1 = valueinput.getDoubleOr("PushZ", EntityMinecartFurnace.DEFAULT_PUSH.z);
