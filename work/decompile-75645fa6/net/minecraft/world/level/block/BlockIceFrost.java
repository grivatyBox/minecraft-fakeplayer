...
public class BlockIceFrost extends BlockIce {
    ...
    protected void tick(IBlockData iblockdata, WorldServer worldserver, BlockPosition blockposition, RandomSource randomsource) {
        if (randomsource.nextInt(3) == 0 || this.fewerNeigboursThan(worldserver, blockposition, 4)) {
            int i = worldserver.dimension() == World.END ? worldserver.getBrightness(EnumSkyBlock.BLOCK, blockposition) : worldserver.getMaxLocalRawBrightness(blockposition);

            if (i > 11 - (Integer) iblockdata.getValue(BlockIceFrost.AGE) - iblockdata.getLightBlock() && this.slightlyMelt(iblockdata, worldserver, blockposition)) {
