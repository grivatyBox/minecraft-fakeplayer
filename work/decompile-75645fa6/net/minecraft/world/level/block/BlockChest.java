...
public class BlockChest extends BlockChestAbstract<TileEntityChest> implements IBlockWaterlogged {
    ...
    public static final MapCodec<BlockChest> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(BuiltInRegistries.SOUND_EVENT.byNameCodec().fieldOf("open_sound").forGetter(BlockChest::getOpenChestSound), BuiltInRegistries.SOUND_EVENT.byNameCodec().fieldOf("close_sound").forGetter(BlockChest::getCloseChestSound), propertiesCodec()).apply(instance, (soundeffect, soundeffect1, blockbase_info) -> {
    ...
    public static BlockPosition getConnectedBlockPos(BlockPosition blockposition, IBlockData iblockdata) {
        EnumDirection enumdirection = getConnectedDirection(iblockdata);
    ...
    public IBlockData getStateForPlacement(BlockActionContext blockactioncontext) {
        ...
        if (enumdirection1.getAxis().isHorizontal() && flag) {
            EnumDirection enumdirection2 = this.candidatePartnerFacing(blockactioncontext.getLevel(), blockactioncontext.getClickedPos(), enumdirection1.getOpposite());
    ...
    protected BlockPropertyChestType getChestType(World world, BlockPosition blockposition, EnumDirection enumdirection) {
        return enumdirection == this.candidatePartnerFacing(world, blockposition, enumdirection.getClockWise()) ? BlockPropertyChestType.LEFT : (enumdirection == this.candidatePartnerFacing(world, blockposition, enumdirection.getCounterClockWise()) ? BlockPropertyChestType.RIGHT : BlockPropertyChestType.SINGLE);
    }
    ...
    @Nullable
    private EnumDirection candidatePartnerFacing(World world, BlockPosition blockposition, EnumDirection enumdirection) {
        IBlockData iblockdata = world.getBlockState(blockposition.relative(enumdirection));

        return this.chestCanConnectTo(iblockdata) && iblockdata.getValue(BlockChest.TYPE) == BlockPropertyChestType.SINGLE ? (EnumDirection) iblockdata.getValue(BlockChest.FACING) : null;
    }
