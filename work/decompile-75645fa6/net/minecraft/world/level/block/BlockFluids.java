...
public class BlockFluids extends Block implements IFluidSource {
    ...
    protected VoxelShape getCollisionShape(IBlockData iblockdata, IBlockAccess iblockaccess, BlockPosition blockposition, VoxelShapeCollision voxelshapecollision) {
        return voxelshapecollision.alwaysCollideWithFluid() ? VoxelShapes.block() : (voxelshapecollision.isAbove(BlockFluids.SHAPE_STABLE, blockposition, true) && (Integer) iblockdata.getValue(BlockFluids.LEVEL) == 0 && voxelshapecollision.canStandOnFluid(iblockaccess.getFluidState(blockposition.above()), iblockdata.getFluidState()) ? BlockFluids.SHAPE_STABLE : VoxelShapes.empty());
    }
