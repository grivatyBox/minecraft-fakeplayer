...
public class TileEntityStructure extends TileEntity implements BoundingBoxRenderable {
    ...
    protected void saveAdditional(ValueOutput valueoutput) {
        ...
        valueoutput.store("rotation", EnumBlockRotation.LEGACY_CODEC, this.rotation);
        valueoutput.store("mirror", EnumBlockMirror.LEGACY_CODEC, this.mirror);
        valueoutput.store("mode", BlockPropertyStructureMode.LEGACY_CODEC, this.mode);
    ...
    protected void loadAdditional(ValueInput valueinput) {
        ...
        int i = MathHelper.clamp(valueinput.getIntOr("posX", TileEntityStructure.DEFAULT_POS.getX()), -48, 48);
        int j = MathHelper.clamp(valueinput.getIntOr("posY", TileEntityStructure.DEFAULT_POS.getY()), -48, 48);
        int k = MathHelper.clamp(valueinput.getIntOr("posZ", TileEntityStructure.DEFAULT_POS.getZ()), -48, 48);

        this.structurePos = new BlockPosition(i, j, k);
        int l = MathHelper.clamp(valueinput.getIntOr("sizeX", TileEntityStructure.DEFAULT_SIZE.getX()), 0, 48);
        int i1 = MathHelper.clamp(valueinput.getIntOr("sizeY", TileEntityStructure.DEFAULT_SIZE.getY()), 0, 48);
        int j1 = MathHelper.clamp(valueinput.getIntOr("sizeZ", TileEntityStructure.DEFAULT_SIZE.getZ()), 0, 48);

        this.structureSize = new BaseBlockPosition(l, i1, j1);
        this.rotation = (EnumBlockRotation) valueinput.read("rotation", EnumBlockRotation.LEGACY_CODEC).orElse(TileEntityStructure.DEFAULT_ROTATION);
        this.mirror = (EnumBlockMirror) valueinput.read("mirror", EnumBlockMirror.LEGACY_CODEC).orElse(TileEntityStructure.DEFAULT_MIRROR);
        this.mode = (BlockPropertyStructureMode) valueinput.read("mode", BlockPropertyStructureMode.LEGACY_CODEC).orElse(BlockPropertyStructureMode.DATA);
    ...
    public void createdBy(EntityLiving entityliving) {
        this.author = entityliving.getPlainTextName();
    }
    ...
    public boolean saveStructure(boolean flag) {
        if (this.structureName != null) {
            ...
            if (world instanceof WorldServer) {
                ...
                return saveStructure(worldserver, this.structureName, blockposition, this.structureSize, this.ignoreEntities, this.author, flag, List.of());
            }
        }
    ...
    public static boolean saveStructure(WorldServer worldserver, MinecraftKey minecraftkey, BlockPosition blockposition, BaseBlockPosition baseblockposition, boolean flag, String s, boolean flag1, List<Block> list) {
        ...
        definedstructure.fillFromWorld(worldserver, blockposition, baseblockposition, !flag, Stream.concat(list.stream(), Stream.of(Blocks.STRUCTURE_VOID)).toList());
    ...
    private void placeStructure(WorldServer worldserver, DefinedStructure definedstructure) {
        ...
        if (SharedConstants.DEBUG_STRUCTURE_EDIT_MODE) {
            BlockPosition.betweenClosed(blockposition, blockposition.offset(this.structureSize)).forEach((blockposition1) -> {
                worldserver.setBlock(blockposition1, Blocks.STRUCTURE_VOID.defaultBlockState(), 2);
            });
        }
