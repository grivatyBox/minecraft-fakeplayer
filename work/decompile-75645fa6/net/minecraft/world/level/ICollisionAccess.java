...
public interface ICollisionAccess extends IBlockAccess {
    ...
    default Iterable<VoxelShape> getPreMoveCollisions(@Nullable Entity entity, AxisAlignedBB axisalignedbb, Vec3D vec3d) {
        List<VoxelShape> list = this.getEntityCollisions(entity, axisalignedbb);
        Iterable<VoxelShape> iterable = this.getBlockCollisionsFromContext(VoxelShapeCollision.withPosition(entity, vec3d.y), axisalignedbb);

        return list.isEmpty() ? iterable : Iterables.concat(list, iterable);
    }

    default Iterable<VoxelShape> getBlockCollisions(@Nullable Entity entity, AxisAlignedBB axisalignedbb) {
        return this.getBlockCollisionsFromContext(entity == null ? VoxelShapeCollision.empty() : VoxelShapeCollision.of(entity), axisalignedbb);
    }

    default Iterable<VoxelShape> getBlockAndLiquidCollisions(@Nullable Entity entity, AxisAlignedBB axisalignedbb) {
        return this.getBlockCollisionsFromContext(entity == null ? VoxelShapeCollision.emptyWithFluidCollisions() : VoxelShapeCollision.of(entity, true), axisalignedbb);
    }

    private Iterable<VoxelShape> getBlockCollisionsFromContext(VoxelShapeCollision voxelshapecollision, AxisAlignedBB axisalignedbb) {
        return () -> {
            return new VoxelShapeSpliterator(this, voxelshapecollision, axisalignedbb, false, (blockposition_mutableblockposition, voxelshape) -> {
                return voxelshape;
            });
        };
    }
