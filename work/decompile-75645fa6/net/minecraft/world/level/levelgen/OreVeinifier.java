...
public final class OreVeinifier {
    ...
    protected static NoiseChunk.c create(DensityFunction densityfunction, DensityFunction densityfunction1, DensityFunction densityfunction2, PositionalRandomFactory positionalrandomfactory) {
        IBlockData iblockdata = SharedConstants.DEBUG_ORE_VEINS ? Blocks.AIR.defaultBlockState() : null;
            ...
            if (k >= 0 && j >= 0) {
                ...
                if (d1 + d2 < (double) 0.4F) {
                    ...
                    if (randomsource.nextFloat() > 0.7F) {
                        ...
                        return (double) randomsource.nextFloat() < d3 && densityfunction2.compute(densityfunction_b) > (double) -0.3F ? (randomsource.nextFloat() < 0.02F ? oreveinifier_a.rawOreBlock : oreveinifier_a.ore) : (SharedConstants.DEBUG_ORE_VEINS ? Blocks.OAK_BUTTON.defaultBlockState() : oreveinifier_a.filler);
                    }
                }
            } else {
                return iblockdata;
