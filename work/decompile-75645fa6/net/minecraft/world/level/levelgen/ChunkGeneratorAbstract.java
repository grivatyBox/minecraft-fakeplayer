...
public final class ChunkGeneratorAbstract extends ChunkGenerator {
    ...
    private static Aquifer.a createFluidPicker(GeneratorSettingBase generatorsettingbase) {
        ...
        return (j, k, l) -> {
            return SharedConstants.DEBUG_DISABLE_FLUID_GENERATION ? aquifer_b2 : (k < Math.min(-54, i) ? aquifer_b : aquifer_b1);
        };
    }
    ...
    public void addDebugScreenInfo(List<String> list, RandomState randomstate, BlockPosition blockposition) {
        ...
        list.add("NoiseRouter T: " + s + " V: " + decimalformat.format(noiserouter.vegetation().compute(densityfunction_e)) + " C: " + decimalformat.format(noiserouter.continents().compute(densityfunction_e)) + " E: " + decimalformat.format(noiserouter.erosion().compute(densityfunction_e)) + " D: " + decimalformat.format(noiserouter.depth().compute(densityfunction_e)) + " W: " + decimalformat.format(d0) + " PV: " + decimalformat.format((double) NoiseRouterData.peaksAndValleys((float) d0)) + " PS: " + decimalformat.format(noiserouter.preliminarySurfaceLevel().compute(densityfunction_e)) + " N: " + decimalformat.format(noiserouter.finalDensity().compute(densityfunction_e)));
    }
    ...
    public void buildSurface(RegionLimitedWorldAccess regionlimitedworldaccess, StructureManager structuremanager, RandomState randomstate, IChunkAccess ichunkaccess) {
        if (!SharedConstants.debugVoidTerrain(ichunkaccess.getPos()) && !SharedConstants.DEBUG_DISABLE_SURFACE) {
    ...
    private IBlockData debugPreliminarySurfaceLevel(NoiseChunk noisechunk, int i, int j, int k, IBlockData iblockdata) {
        if (SharedConstants.DEBUG_AQUIFERS && k >= 0 && k % 4 == 0) {
            ...
            if (j == i1) {
                iblockdata = i1 < this.getSeaLevel() ? Blocks.SLIME_BLOCK.defaultBlockState() : Blocks.HONEY_BLOCK.defaultBlockState();
            }
        }
