...
public class NoiseChunk implements DensityFunction.a, DensityFunction.b {
        ...
        if (!blender.isEmpty()) {
            for (int l = 0; l <= this.noiseSizeXZ; ++l) {
                ...
                for (int k1 = 0; k1 <= this.noiseSizeXZ; ++k1) {
                    ...
                    this.blendAlpha.values[l + k1 * this.blendAlpha.sizeXZ] = blender_a.alpha();
                    this.blendOffset.values[l + k1 * this.blendOffset.sizeXZ] = blender_a.blendingOffset();
                }
            }
        } else {
    ...
    public int maxPreliminarySurfaceLevel(int i, int j, int k, int l) {
        ...
        for (int j1 = j; j1 <= l; j1 += 4) {
            for (int k1 = i; k1 <= k; k1 += 4) {
    ...
    public int preliminarySurfaceLevel(int i, int j) {
        ...
        return this.preliminarySurfaceLevelCache.computeIfAbsent(BlockPosition2D.asLong(k, l), this::computePreliminarySurfaceLevel);
    }
    ...
    private int computePreliminarySurfaceLevel(long i) {
        ...
        return MathHelper.floor(this.preliminarySurfaceLevel.compute(new DensityFunction.e(j, 0, k)));
    }
    ...
    private DensityFunction wrapNew(DensityFunction densityfunction) {
        if (densityfunction instanceof DensityFunctions.m) {
            DensityFunctions.m densityfunctions_m = (DensityFunctions.m) densityfunction;
            Object object;

            switch (densityfunctions_m.type()) {
                case Interpolated:
                    object = new NoiseChunk.i(densityfunctions_m.wrapped());
                    break;
                case FlatCache:
                    object = new NoiseChunk.g(densityfunctions_m.wrapped(), true);
                    break;
                case Cache2D:
                    object = new NoiseChunk.d(densityfunctions_m.wrapped());
                    break;
                case CacheOnce:
                    object = new NoiseChunk.f(densityfunctions_m.wrapped());
                    break;
                case CacheAllInCell:
                    object = new NoiseChunk.e(densityfunctions_m.wrapped());
                    break;
                default:
            ...
            } else if (densityfunction instanceof DensityFunctions.k) {
                DensityFunctions.k densityfunctions_k = (DensityFunctions.k) densityfunction;

                return (DensityFunction) densityfunctions_k.function().value();
            } else {
    ...
    private class g implements DensityFunctions.n, NoiseChunk.h {
            ...
            if (flag) {
                for (int i = 0; i <= NoiseChunk.this.noiseSizeXZ; ++i) {
                    ...
                    for (int l = 0; l <= NoiseChunk.this.noiseSizeXZ; ++l) {
                        ...
                        this.values[i + l * this.sizeXZ] = densityfunction.compute(new DensityFunction.e(k, 0, j1));
                    }
                }
            }
        ...
        public double compute(DensityFunction.b densityfunction_b) {
            ...
            return k >= 0 && l >= 0 && k < this.sizeXZ && l < this.sizeXZ ? this.values[k + l * this.sizeXZ] : this.noiseFiller.compute(densityfunction_b);
        }
    ...
    private class e implements DensityFunctions.n, NoiseChunk.h {
    ...
    public class i implements DensityFunctions.n, NoiseChunk.h {
    ...
    private class f implements DensityFunctions.n, NoiseChunk.h {
    ...
    private static class d implements DensityFunctions.n, NoiseChunk.h {
