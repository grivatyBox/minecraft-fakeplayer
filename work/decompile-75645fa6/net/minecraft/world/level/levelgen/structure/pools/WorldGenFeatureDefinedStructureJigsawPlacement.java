...
public class WorldGenFeatureDefinedStructureJigsawPlacement {
    ...
    public static Optional<Structure.b> addPieces(Structure.a structure_a, Holder<WorldGenFeatureDefinedStructurePoolTemplate> holder, Optional<MinecraftKey> optional, int i, BlockPosition blockposition, boolean flag, Optional<HeightMap.Type> optional1, JigsawStructure.a jigsawstructure_a, PoolAliasLookup poolaliaslookup, DimensionPadding dimensionpadding, LiquidSettings liquidsettings) {
        ...
        if (worldgenfeaturedefinedstructurepoolstructure == WorldGenFeatureDefinedStructurePoolEmpty.INSTANCE) {
            ...
            int j = (structureboundingbox.maxX() + structureboundingbox.minX()) / 2;
            int k = (structureboundingbox.maxZ() + structureboundingbox.minZ()) / 2;
            int l = optional1.isEmpty() ? blockposition2.getY() : blockposition.getY() + chunkgenerator.getFirstFreeHeight(j, k, (HeightMap.Type) optional1.get(), levelheightaccessor, structure_a.randomState());
            int i1 = structureboundingbox.minY() + worldgenfeaturepillageroutpostpoolpiece.getGroundLevelDelta();

            worldgenfeaturepillageroutpostpoolpiece.move(0, l - i1, 0);
            if (isStartTooCloseToWorldHeightLimits(levelheightaccessor, dimensionpadding, worldgenfeaturepillageroutpostpoolpiece.getBoundingBox())) {
                ...
                int j1 = l + baseblockposition.getY();

                return Optional.of(new Structure.b(new BlockPosition(j, j1, k), (structurepiecesbuilder) -> {
                    List<WorldGenFeaturePillagerOutpostPoolPiece> list = Lists.newArrayList();

                    list.add(worldgenfeaturepillageroutpostpoolpiece);
                    if (i > 0) {
                        AxisAlignedBB axisalignedbb = new AxisAlignedBB((double) (j - jigsawstructure_a.horizontal()), (double) Math.max(j1 - jigsawstructure_a.vertical(), levelheightaccessor.getMinY() + dimensionpadding.bottom()), (double) (k - jigsawstructure_a.horizontal()), (double) (j + jigsawstructure_a.horizontal() + 1), (double) Math.min(j1 + jigsawstructure_a.vertical() + 1, levelheightaccessor.getMaxY() + 1 - dimensionpadding.top()), (double) (k + jigsawstructure_a.horizontal() + 1));
    ...
    public static boolean generateJigsaw(WorldServer worldserver, Holder<WorldGenFeatureDefinedStructurePoolTemplate> holder, MinecraftKey minecraftkey, int i, BlockPosition blockposition, boolean flag) {
        ...
        Optional<Structure.b> optional = addPieces(structure_a, holder, Optional.of(minecraftkey), i, blockposition, false, Optional.empty(), new JigsawStructure.a(128), PoolAliasLookup.EMPTY, JigsawStructure.DEFAULT_DIMENSION_PADDING, JigsawStructure.DEFAULT_LIQUID_SETTINGS);
