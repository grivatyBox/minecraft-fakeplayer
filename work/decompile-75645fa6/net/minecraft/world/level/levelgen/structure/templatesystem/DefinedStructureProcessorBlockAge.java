...
public class DefinedStructureProcessorBlockAge extends DefinedStructureProcessor {
    ...
    public DefinedStructure.BlockInfo processBlock(IWorldReader iworldreader, BlockPosition blockposition, BlockPosition blockposition1, DefinedStructure.BlockInfo definedstructure_blockinfo, DefinedStructure.BlockInfo definedstructure_blockinfo1, DefinedStructureInfo definedstructureinfo) {
        ...
        if (!iblockdata.is(Blocks.STONE_BRICKS) && !iblockdata.is(Blocks.STONE) && !iblockdata.is(Blocks.CHISELED_STONE_BRICKS)) {
            if (iblockdata.is(TagsBlock.STAIRS)) {
                iblockdata1 = this.maybeReplaceStairs(iblockdata, randomsource);
            } else if (iblockdata.is(TagsBlock.SLABS)) {
                iblockdata1 = this.maybeReplaceSlab(iblockdata, randomsource);
            } else if (iblockdata.is(TagsBlock.WALLS)) {
                iblockdata1 = this.maybeReplaceWall(iblockdata, randomsource);
    ...
    @Nullable
    private IBlockData maybeReplaceStairs(IBlockData iblockdata, RandomSource randomsource) {
        if (randomsource.nextFloat() >= 0.5F) {
            return null;
        } else {
            IBlockData[] aiblockdata = new IBlockData[]{Blocks.MOSSY_STONE_BRICK_STAIRS.withPropertiesOf(iblockdata), Blocks.MOSSY_STONE_BRICK_SLAB.defaultBlockState()};
    ...
    @Nullable
    private IBlockData maybeReplaceSlab(IBlockData iblockdata, RandomSource randomsource) {
        return randomsource.nextFloat() < this.mossiness ? Blocks.MOSSY_STONE_BRICK_SLAB.withPropertiesOf(iblockdata) : null;
    }

    @Nullable
    private IBlockData maybeReplaceWall(IBlockData iblockdata, RandomSource randomsource) {
        return randomsource.nextFloat() < this.mossiness ? Blocks.MOSSY_STONE_BRICK_WALL.withPropertiesOf(iblockdata) : null;
    }
