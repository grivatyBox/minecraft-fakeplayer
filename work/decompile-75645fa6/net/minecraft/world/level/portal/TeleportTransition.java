...
public record TeleportTransition(WorldServer newLevel, Vec3D position, Vec3D deltaMovement, float yRot, float xRot, boolean missingRespawnBlock, boolean asPassenger, Set<Relative> relatives, TeleportTransition.a postTeleportTransition) {
    ...
    public static TeleportTransition createDefault(EntityPlayer entityplayer, TeleportTransition.a teleporttransition_a) {
        WorldServer worldserver = entityplayer.level().getServer().findRespawnDimension();
        WorldData.a worlddata_a = worldserver.getRespawnData();

        return new TeleportTransition(worldserver, findAdjustedSharedSpawnPos(worldserver, entityplayer), Vec3D.ZERO, worlddata_a.yaw(), worlddata_a.pitch(), false, false, Set.of(), teleporttransition_a);
    }

    public static TeleportTransition missingRespawnBlock(EntityPlayer entityplayer, TeleportTransition.a teleporttransition_a) {
        WorldServer worldserver = entityplayer.level().getServer().findRespawnDimension();
        WorldData.a worlddata_a = worldserver.getRespawnData();

        return new TeleportTransition(worldserver, findAdjustedSharedSpawnPos(worldserver, entityplayer), Vec3D.ZERO, worlddata_a.yaw(), worlddata_a.pitch(), true, false, Set.of(), teleporttransition_a);
    }

    private static Vec3D findAdjustedSharedSpawnPos(WorldServer worldserver, Entity entity) {
        return entity.adjustSpawnLocation(worldserver, worldserver.getRespawnData().pos()).getBottomCenter();
    }
