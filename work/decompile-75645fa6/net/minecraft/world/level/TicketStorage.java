...
public class TicketStorage extends PersistentBase {
    ...
    public boolean addTicket(long i, Ticket ticket) {
        ...
        if (SharedConstants.DEBUG_VERBOSE_SERVER_EVENTS) {
            TicketStorage.LOGGER.debug("ATI {} {}", new ChunkCoordIntPair(i), ticket);
        }
        ...
        if (list == null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (isTicketSameTypeAndLevel(ticket, ticket1)) {
                    iterator.remove();
                    if (SharedConstants.DEBUG_VERBOSE_SERVER_EVENTS) {
                        TicketStorage.LOGGER.debug("RTI {} {}", new ChunkCoordIntPair(i), ticket1);
                    }
    ...
    public void purgeStaleTickets(PlayerChunkMap playerchunkmap) {
    ...
    private boolean canTicketExpire(PlayerChunkMap playerchunkmap, Ticket ticket, long i) {
        if (!ticket.getType().hasTimeout()) {
            return false;
        } else if (ticket.getType().canExpireIfUnloaded()) {
            return true;
        } else {
            PlayerChunk playerchunk = playerchunkmap.getUpdatingChunkIfPresent(i);
    ...
    public void removeTicketIf(TicketStorage.b ticketstorage_b, @Nullable Long2ObjectOpenHashMap<List<Ticket>> long2objectopenhashmap) {
        ...
        while (objectiterator.hasNext()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (ticketstorage_b.test(ticket, i)) {
                    if (long2objectopenhashmap != null) {
                        List<Ticket> list = (List) long2objectopenhashmap.computeIfAbsent(i, (j) -> {
    ...
    public interface b {
        ...
        boolean test(Ticket ticket, long i);
    }
}
