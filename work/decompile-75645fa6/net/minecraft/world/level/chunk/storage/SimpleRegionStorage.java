...
public class SimpleRegionStorage implements AutoCloseable {
    ...
    public NBTTagCompound upgradeChunkTag(NBTTagCompound nbttagcompound, int i) {
        ...
        NBTTagCompound nbttagcompound1 = this.dataFixType.updateToCurrentVersion(this.fixerUpper, nbttagcompound, j);

        return GameProfileSerializer.addCurrentDataVersion(nbttagcompound1);
    }

    public Dynamic<NBTBase> upgradeChunkTag(Dynamic<NBTBase> dynamic, int i) {
        int j = GameProfileSerializer.getDataVersion(dynamic, i);
        Dynamic<NBTBase> dynamic1 = this.dataFixType.updateToCurrentVersion(this.fixerUpper, dynamic, j);

        return GameProfileSerializer.addCurrentDataVersion(dynamic1);
    }
