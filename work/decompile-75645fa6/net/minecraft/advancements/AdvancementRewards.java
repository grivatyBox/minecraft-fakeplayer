...
public record AdvancementRewards(int experience, List<ResourceKey<LootTable>> loot, List<ResourceKey<IRecipe<?>>> recipes, Optional<CacheableFunction> function) {
    ...
    public void grant(EntityPlayer entityplayer) {
        ...
        LootParams lootparams = (new LootParams.a(worldserver)).withParameter(LootContextParameters.THIS_ENTITY, entityplayer).withParameter(LootContextParameters.ORIGIN, entityplayer.position()).create(LootContextParameterSets.ADVANCEMENT_REWARD);
        boolean flag = false;
        ...
        for (ResourceKey<LootTable> resourcekey : this.loot) {
            ...
            while (objectlistiterator.hasNext()) {
                ...
                if (entityplayer.addItem(itemstack)) {
                    worldserver.playSound((Entity) null, entityplayer.getX(), entityplayer.getY(), entityplayer.getZ(), SoundEffects.ITEM_PICKUP, SoundCategory.PLAYERS, 0.2F, ((entityplayer.getRandom().nextFloat() - entityplayer.getRandom().nextFloat()) * 0.7F + 1.0F) * 2.0F);
                    flag = true;
                } else {
