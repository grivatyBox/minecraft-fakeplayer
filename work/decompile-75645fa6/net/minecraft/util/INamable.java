...
public interface INamable {
    ...
    static <T extends INamable> Function<String, T> createNameLookup(T[] at) {
        return createNameLookup(at, INamable::getSerializedName);
    }

    static <T> Function<String, T> createNameLookup(T[] at, Function<T, String> function) {
        if (at.length > 16) {
            Map<String, T> map = (Map) Arrays.stream(at).collect(Collectors.toMap(function, (object) -> {
                return object;
            }));
            ...
            return (s) -> {
                for (T t0 : at) {
                    if (((String) function.apply(t0)).equals(s)) {
                        return t0;
                    }
                }
