...
public class DebugSubscriptions<T> {
    ...
    public static final DebugSubscription<List<BlockPosition>> RAIDS = registerWithValue("raids", BlockPosition.STREAM_CODEC.apply(ByteBufCodecs.list()));
    ...
    public static final DebugSubscription<BlockPosition> NEIGHBOR_UPDATES = registerTemporaryValue("neighbor_updates", BlockPosition.STREAM_CODEC, 200);
    ...
    public static DebugSubscription<?> bootstrap(IRegistry<DebugSubscription<?>> iregistry) {
        return DebugSubscriptions.DEDICATED_SERVER_TICK_TIME;
    }

    private static DebugSubscription<?> registerSimple(String s) {
        return (DebugSubscription) IRegistry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, MinecraftKey.withDefaultNamespace(s), new DebugSubscription((StreamCodec) null));
    }

    private static <T> DebugSubscription<T> registerWithValue(String s, StreamCodec<? super RegistryFriendlyByteBuf, T> streamcodec) {
        return (DebugSubscription) IRegistry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, MinecraftKey.withDefaultNamespace(s), new DebugSubscription(streamcodec));
    }

    private static <T> DebugSubscription<T> registerTemporaryValue(String s, StreamCodec<? super RegistryFriendlyByteBuf, T> streamcodec, int i) {
        return (DebugSubscription) IRegistry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, MinecraftKey.withDefaultNamespace(s), new DebugSubscription(streamcodec, i));
    }
}
