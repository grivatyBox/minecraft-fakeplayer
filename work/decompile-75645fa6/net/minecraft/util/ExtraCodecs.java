...
public class ExtraCodecs {
    ...
    public static final Codec<NBTBase> NBT = converter(DynamicOpsNBT.INSTANCE);
    public static final Codec<Vector2f> VECTOR2F = Codec.FLOAT.listOf().comapFlatMap((list) -> {
        ...
        return List.of(vector2f.x(), vector2f.y());
    });
    ...
    public static final Codec<Vector3i> VECTOR3I = Codec.INT.listOf().comapFlatMap((list) -> {
        return SystemUtils.fixedSize(list, 3).map((list1) -> {
            return new Vector3i((Integer) list1.get(0), (Integer) list1.get(1), (Integer) list1.get(2));
        });
    }, (vector3i) -> {
        return List.of(vector3i.x(), vector3i.y(), vector3i.z());
    });
    ...
    private static MapCodec<GameProfile> gameProfileCodec(Codec<UUID> codec) {
        return RecordCodecBuilder.mapCodec((instance) -> {
            return instance.group(codec.fieldOf("id").forGetter(GameProfile::id), ExtraCodecs.PLAYER_NAME.fieldOf("name").forGetter(GameProfile::name), ExtraCodecs.PROPERTY_MAP.optionalFieldOf("properties", PropertyMap.EMPTY).forGetter(GameProfile::properties)).apply(instance, GameProfile::new);
        });
    }
