...
public class TileEntityJigsaw extends TileEntity {
    ...
    protected void saveAdditional(ValueOutput valueoutput) {
        ...
        valueoutput.store("name", MinecraftKey.CODEC, this.name);
        valueoutput.store("target", MinecraftKey.CODEC, this.target);
        valueoutput.store("pool", TileEntityJigsaw.POOL_CODEC, this.pool);
        valueoutput.putString("final_state", this.finalState);
        valueoutput.store("joint", TileEntityJigsaw.JointType.CODEC, this.joint);
    ...
    protected void loadAdditional(ValueInput valueinput) {
        ...
        this.name = (MinecraftKey) valueinput.read("name", MinecraftKey.CODEC).orElse(TileEntityJigsaw.EMPTY_ID);
        this.target = (MinecraftKey) valueinput.read("target", MinecraftKey.CODEC).orElse(TileEntityJigsaw.EMPTY_ID);
        this.pool = (ResourceKey) valueinput.read("pool", TileEntityJigsaw.POOL_CODEC).orElse(WorldGenFeaturePieces.EMPTY);
        this.finalState = valueinput.getStringOr("final_state", "minecraft:air");
        this.joint = (TileEntityJigsaw.JointType) valueinput.read("joint", TileEntityJigsaw.JointType.CODEC).orElseGet(() -> {
