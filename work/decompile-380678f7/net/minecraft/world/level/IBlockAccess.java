        ...
        if (vec3d2.lengthSqr() < (double) MathHelper.square(0.99999F)) {
            ...
            if (i < 0) {
                ...
                for (BlockPosition blockposition1 : BlockPosition.betweenClosed(axisalignedbb)) {
                    if (!longset.contains(blockposition1.asLong()) && !iblockaccess_a.visit(blockposition1, i + 1)) {
                        return false;
                    }
                }
                ...
                for (int k2 = i; k2 <= l1; ++k2) {
                    for (int l2 = j; l2 <= i2; ++l2) {
                        for (int i3 = k; i3 <= j2; ++i3) {
                            if (longset.add(BlockPosition.asLong(k2, l2, i3)) && !iblockaccess_a.visit(blockposition_mutableblockposition.set(k2, l2, i3), k1)) {
                                return -1;
                            }
                        }
                    }
