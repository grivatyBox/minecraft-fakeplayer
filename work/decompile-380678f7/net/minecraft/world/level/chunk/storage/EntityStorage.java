...
public class EntityStorage implements EntityPersistentStorage<Entity> {
        ...
        if (this.emptyChunks.contains(chunkcoordintpair.toLong())) {
            ...
            Function function = (optional) -> {
                if (optional.isEmpty()) {
                    ...
                    try (ProblemReporter.j problemreporter_j = new ProblemReporter.j(IChunkAccess.problemPath(chunkcoordintpair), EntityStorage.LOGGER)) {
                        ValueInput valueinput = TagValueInput.create(problemreporter_j, this.level.registryAccess(), nbttagcompound);
                        ValueInput.b valueinput_b = valueinput.childrenListOrEmpty("Entities");
                        List<Entity> list = EntityTypes.loadEntitiesRecursive(valueinput_b, this.level, EntitySpawnReason.LOAD).toList();
    ...
    private static ChunkEntities<Entity> emptyChunk(ChunkCoordIntPair chunkcoordintpair) {
        return new ChunkEntities<Entity>(chunkcoordintpair, List.of());
    }
        ...
        if (chunkentities.isEmpty()) {
            ...
            try (ProblemReporter.j problemreporter_j = new ProblemReporter.j(IChunkAccess.problemPath(chunkcoordintpair), EntityStorage.LOGGER)) {
                ...
                chunkentities.getEntities().forEach((entity) -> {
                    TagValueOutput tagvalueoutput = TagValueOutput.createWithContext(problemreporter_j.forChild(entity.problemPath()), entity.registryAccess());
