...
public class LootSelector {
    ...
    public void validate(LootCollector lootcollector) {
        for (int i = 0; i < this.conditions.size(); ++i) {
            ((LootItemCondition) this.conditions.get(i)).validate(lootcollector.forChild(new ProblemReporter.d("conditions", i)));
        }
        ...
        for (int k = 0; k < this.entries.size(); ++k) {
            ((LootEntryAbstract) this.entries.get(k)).validate(lootcollector.forChild(new ProblemReporter.d("entries", k)));
        }

        this.rolls.validate(lootcollector.forChild(new ProblemReporter.c("rolls")));
        this.bonusRolls.validate(lootcollector.forChild(new ProblemReporter.c("bonus_rolls")));
    }
