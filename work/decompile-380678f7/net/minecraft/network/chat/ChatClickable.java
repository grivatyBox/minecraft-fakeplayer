...
public interface ChatClickable {
    ...
    public static record h(Holder<Dialog> dialog) implements ChatClickable {

        public static final MapCodec<ChatClickable.h> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
            return instance.group(Dialog.CODEC.fieldOf("dialog").forGetter(ChatClickable.h::dialog)).apply(instance, ChatClickable.h::new);
        });

        @Override
        public ChatClickable.EnumClickAction action() {
            return ChatClickable.EnumClickAction.SHOW_DIALOG;
        }
    }
    ...
    public static record d(MinecraftKey id, Optional<NBTBase> payload) implements ChatClickable {

        public static final MapCodec<ChatClickable.d> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
            return instance.group(MinecraftKey.CODEC.fieldOf("id").forGetter(ChatClickable.d::id), ExtraCodecs.NBT.optionalFieldOf("payload").forGetter(ChatClickable.d::payload)).apply(instance, ChatClickable.d::new);
        });

        @Override
        public ChatClickable.EnumClickAction action() {
            return ChatClickable.EnumClickAction.CUSTOM;
        }
    }

    public static enum EnumClickAction implements INamable {

        OPEN_URL("open_url", true, ChatClickable.OpenUrl.CODEC), OPEN_FILE("open_file", false, ChatClickable.OpenFile.CODEC), RUN_COMMAND("run_command", true, ChatClickable.RunCommand.CODEC), SUGGEST_COMMAND("suggest_command", true, ChatClickable.SuggestCommand.CODEC), SHOW_DIALOG("show_dialog", true, ChatClickable.h.CODEC), CHANGE_PAGE("change_page", true, ChatClickable.ChangePage.CODEC), COPY_TO_CLIPBOARD("copy_to_clipboard", true, ChatClickable.CopyToClipboard.CODEC), CUSTOM("custom", true, ChatClickable.d.CODEC);
