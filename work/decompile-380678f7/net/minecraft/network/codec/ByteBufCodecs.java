...
public interface ByteBufCodecs {
    ...
    StreamCodec<ByteBuf, Integer> RGB_COLOR = new StreamCodec<ByteBuf, Integer>() {
        public Integer decode(ByteBuf bytebuf) {
            return ARGB.color(bytebuf.readByte() & 255, bytebuf.readByte() & 255, bytebuf.readByte() & 255);
        }
        ...
        return (streamcodec) -> {
            return new StreamCodec<B, V>() {
                public V decode(B b0) {
                    T t0 = (T) streamcodec.decode(b0);

                    return (V) codec.parse(dynamicops, t0).getOrThrow((s) -> {
                        return new DecoderException("Failed to decode: " + s + " " + String.valueOf(t0));
                    });
                }

                public void encode(B b0, V v0) {
                    T t0 = (T) codec.encodeStart(dynamicops, v0).getOrThrow((s) -> {
                        return new EncoderException("Failed to encode: " + s + " " + String.valueOf(v0));
                    });

                    streamcodec.encode(b0, t0);
                }
            };
        };
    }
    ...
    static <T> StreamCodec<ByteBuf, T> fromCodec(Codec<T> codec, Supplier<NBTReadLimiter> supplier) {
        return tagCodec(supplier).apply(fromCodec(DynamicOpsNBT.INSTANCE, codec));
    }
    ...
    static <V> StreamCodec.a<ByteBuf, V, V> lengthPrefixed(int i) {
        return lengthPrefixed(i, (bytebuf, bytebuf1) -> {
            return bytebuf1;
        });
    }
    ...
    static StreamCodec<ByteBuf, JsonElement> lenientJson(final int i) {
        return new StreamCodec<ByteBuf, JsonElement>() {
            ...
            public JsonElement decode(ByteBuf bytebuf) {
                String s = Utf8String.read(bytebuf, i);
            ...
            public void encode(ByteBuf bytebuf, JsonElement jsonelement) {
                String s = null.GSON.toJson(jsonelement);
